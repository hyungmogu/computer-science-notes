...
\begin{enumerate}[a.]
    ...
    Now, we need to determine the cost of outer loop.

    \bigskip

    Because we know the outer loop runs from $i = 1$ until $i = n-1$ with $i$ increasing
    by $2^i$ per iteration, we can conclude this is the same as the loop starting
    from $i' = 0$ until $i' = \lceil \log (n - 1) \rceil$ with $i'$ increasing by
    1 per iteration.

    \bigskip

    Because we know the outer loop takes $i$ steps per iteration,
    we can conclude that in terms of $i'$, the loop takes $2^{i'}$ steps per iteration

    \bigskip

    Using this fact, we can calculate that the outer loop has cost of

    \begin{align}
        \sum\limits_{i'=1}^{\lceil \log (n-1) \rceil} 2^{i'} &=
    \end{align}
