...
\begin{enumerate}[a.]
    ...
    Because we know the loop will terminate when \textbf{if lst[i] == lst[j]} and because we know
    this condition occurs when $i = n-2$, we can conclude the loop will start
    at $i = 0$ and run until $i = n - 2$.

    \bigskip

    Because we know the inner loop condition \textbf{for j in range(i+1,n)}
    stays true until $i = n - 2$ even for the worst case, we can conclude that the cost of
    ...
    Since the outer most loop starts at $i = 0$ and ends at $i = n-1$ with each taking
    $(n - i -1)$ steps, the outer most loop has cost of
    ...
    Then, we can conclude that under this input group, the algorithm has runtime of
    $\Omega(n^2)$.

    \bigskip

    Since both $\mathcal{O}(n^2)$ and $\Omega(n^2)$ are true, we can conclude
    $\Theta(n^2)$ is also true.
