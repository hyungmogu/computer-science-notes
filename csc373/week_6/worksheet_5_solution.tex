...
\begin{enumerate}[1.]
    ...
    I must show such that there is no flow
    at vertex $u$. That is, there exists a maximum flow $f$ in $G$ such that
    ...
    \begin{enumerate}[1.]
        \item \textbf{Case 1:} When $u$ only has no flow going in and out
        ...
        \begin{mdframed}
        Assume $u$ only has no flow going in and out.
        ...
        Then, we know by the edge case of flow conservation that $f(u,v) = 0$ for all $v \in V$.
        ...
        \begin{align}
            \sum\limits_{v \in V} f(u,v) = \sum\limits_{v \in V} f(v,u) &= 0 & [\text{By flow conservation}]
        \end{align}

        \bigskip

        Then, using the capacity constraint ($\forall a,b \in V$, $0 \leq f(a,b)$), we can write
        $f(v,u)$ in $\sum\limits_{v \in V} f(v,u)$ must be 0 for all $v \in V$.
