...
\begin{enumerate}[1.]
    ...
    \begin{itemize}
        ...
        We first note that a directed graph is connected if there is a path between
        every pair of verticies.

        Thus, the edge connectivity implies the minimum number of edges required
        to remove a vertex from graph $G'$.

        So, I now show the minimum number of edges that must be removed to disconnect the graph.

        \bigskip

        I note that a directed graph is connected if there is a path between
        every pair of verticies.

        \bigskip

        So I do so by using maximum flow algorithm to find the vertex $u$ with the minimum amount
        of flow, and then cutting those edges to make the graph disconnected.
    ...
    \begin{itemize}
        ...
        \item I wonder what does this maximum-flow algorithm do.
        \item I am wondering how I can
