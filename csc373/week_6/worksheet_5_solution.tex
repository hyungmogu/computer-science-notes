...
\begin{enumerate}[1.]
    ...
    \begin{itemize}
        \item Combine all sources $s_i$ into a single source $s$ with outgoing flow $\sum\limits_{i} p_i$
        \item Combine all sinks $t_j$ into a single sink $t$ with incoming flow $\sum\limits_{j} q_j$
        \item Connect source $s$ to $v$ with an edge having the sum of relevant edge weights before combining as its weight
        \item Connect source $v$ to $t$ with an edge having the sum of relevant edge weights before combining as its weight
    ...
    \begin{itemize}
        ...
        \begin{itemize}
            \item 한국어로 '거꾸로 가는 수압 decrease 해서 앞으로 가는 수압 더 쎄게 만들기'
