...
\begin{enumerate}[1.]
    ...
    \begin{itemize}
        ...
        \textbf{Predicate Logic:} $\forall f \in G$, $\forall f' \in G_f$, $\forall (u,v) \in E \text{ where $u \in V - \{s,t\},v \in V$}$,
        $0 \leq (f \uparrow f')(u,v) \land (f \uparrow f')(u,v) \leq c_{f \uparrow f'}(u,v)$

        \bigskip

        Let $G = (V,E)$ be a flow network with sources $s$ and sink $t$. Let $f$
        be a flow in $G$. Let $G_f$ be a residual network of $G$ induced by $f$ and let $f'$ be
        a flow in $G_f$. Let $(u,v)$ be an edge in $E$ where $u \in V - \{s,t\}$ and $v \in V$.
