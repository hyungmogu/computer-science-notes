...
\begin{enumerate}[1.]
    ...
    We know that in addition to counting $v_i$ where $i = 1, .., n$ in adjacency list,
    we are counting $\lvert Adj[v_i] \rvert$ edges.

    \bigskip

    Since we are traversing $\lvert V \rvert = n$ many vertices, we can write that
    in addition to  there are total of
    $\sum\limits_{i=1}^{n} \lvert Adj[v_i] \rvert = \lvert E \rvert$ edges.
