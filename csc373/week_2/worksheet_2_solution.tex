...
\begin{enumerate}[1.]
    ...
    \begin{itemize}
        ...
        The greedy algorithm works by assigning $a_{\text{min}} = min(a_{\text{min}}, S_i)$ and\\
        $a_{\text{max}} = min(a_{\text{max}}, S_i)$

        \bigskip

        Let $A = [a_{\text{min}}, a_{\text{max}}]$ be the solution $I' = (n - 1, \{a_1, ..., a_{n-1}\})$.
