...
\begin{enumerate}[1.]
    ...
    \begin{itemize}
        ...
        \begin{mdframed}
        Let $i$ be the index in the set of elements.

        \bigskip

        Let $A = [a_{\text{min}}, a_{\text{max}}]$ be the solution. The greedy algorithm works by assigning $a_{\text{min}} = min(a_{\text{min}}, x_n)$ and
        $a_{\text{max}} = max(a_{\text{max}}, x_n)$, and then continuing by solving the subproblem

        \begin{align}
            I' = (n - 1, \{x_1,..., x_{n-1}\}, S - x_n)
        \end{align}
