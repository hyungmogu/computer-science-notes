...
\begin{enumerate}[1.]
    ...
    \begin{itemize}
        ...
        \begin{mdframed}
        $\textbf{not\_followed(uid)} \coloneqq \Pi_{followers, followed, start}(\sigma_{\substack{User.uid = Follows.followed \land\\ User.uid \neq Follows.follower}}(Follows)) - Follows$
        \end{mdframed}

        \item Find all users who liked a post they do not follow

        \begin{mdframed}
        $\textbf{liked\_post\_not\_followed(liker, uid)} \coloneqq \Pi_{liker, uid}(Post \bowtie Likes) -\\ \Pi_{liker, uid}(\rho_{followers \to liker, followed \to uid}(Follows))$
        \end{mdframed}
