...
\begin{itemize}
    ...
    \begin{lstlisting}[language=Java,caption={lesson\_04/Game.java}]
    public class Game {
        ...
        public boolean applyGuess(String letters) { // <- this little devil here :)
            if (letters.length() == 0) { // <- 1. notice checks for empty string. If so, raise error
    ...
    \begin{lstlisting}[language=Java,caption={lesson\_04/Prompter.java}]
    public class Prompter {
        ...
        ...
        public boolean promptForGuess() {
            ...
            try {
                isHit = game.applyGuess(guessInput); // <- 2. charAt() is removed
                isAcceptable = true;
            } catch (IllegalArgumentException iae) {
                System.out.println(iae.getMessage());
            }
