...
\begin{enumerate}[1.]
    ...
    class Q4b {
        public static void main(String[ ] args) {
            ...
            try {
                ...
                while(sunkResults.next()) {
                    country = sunkResults.getString(1);
                    count = sunkResults.getInt(1);
                ...
                PrepareStatement damagedStat = myCon.prepareStatement("SELECT country, COUNT(Outcomes.result) FROM Classes " +
                                                                    "INNER JOIN Ships ON Classes.class = Ships.class " +
                                                                    "INNER JOIN Outcomes ON Ships.name = Outcomes.ship " +
                                                                    "INNER JOIN Battles ON Battles.name = Outcome.battle " +
                                                                    "GROUP BY country " +
                    ...
                    sunkStat.setString(1, targetBattle);
                ResultSet damagedResults = damagedStat.executeQuery();

                while(damagedResults.next()) {
                    country = damagedResults.getString(1);
                    count = damagedResults.getInt(1);
    ...
    class Q4d {
        public static void main(String[ ] args) {
            ...
            Date dateLaunched;
            String newDateLaunched;
            Date dateBattle;
            String newDateBattle;
            ...
            try {
                ...
                PrepareStatement execStat = myCon.prepareStatement("SELECT Ships.name, Ships.class, Outcomes.battle " +
                                "FROM Ships " +
                                "INNER JOIN Outcomes ON Ships.name = Outcomes.ship " +
                                "INNER JOIN Battles ON Outcomes.battle = Battles.name " +
                ...
                while (results.next()) {
                    shipName = results.getString(1);
                    class = results.getString(2);
                    battle = results.getString(3);

                    System.out.println("Error. Ship %s is launched after date of battle:");
                    System.out.println("Enter correct launched date (YYYY-MM-DD. Type enter to skip):");
                    newDateLaunched = scanObj.nextLine();

                    if (newDateLaunched.trim().equals("")) {
                        PrepareStatement updateShipsStat  = myCon.prepareStatement("UPDATE Ships " +
                                        "SET launched = ? " +
                                        "WHERE name = ? AND class = ?");

                            updateShipsStat.setString(1, newDateLaunched);
                            updateShipsStat.setString(2, shipName);
                            updateShipsStat.setString(3, class);
                        updateShipsStat.executeQuery();
                    }

                    System.out.println("Enter correct launched date (YYYY-MM-DD. Type enter to skip):");
                    newDateBattle = scanObj.nextLine();

                    if (newDateBattle.trim().equals("")) {
                        PrepareStatement updateBatlleStat  = myCon.prepareStatement("UPDATE Battles " +
                                        "SET launched = ? " +
                                        "WHERE name = ?");
                            updateBatlleStat.setString(1, newDateBattle);
                            updateBatlleStat.setString(2, battle);
                        updateShipsStat.executeQuery();
                    }
