...
\begin{enumerate}[1.]
    ...
    void countryWithMostShipsSunk() {
        ...
        EXEC SQL DECLARE execCursor CURSOR FOR
            ...
            INNER JOIN (
                SELECT * FROM FROM Ships
                INNER JOIN (
                    SELECT * FROM Battles
                    INNER JOIN Outcomes ON Battle.name = Outcomes.battle;
                ) AS BattleOutcome ON BattleOutcome.ship = Ships.name;
            ) AS Temp ON Classes.class = Temp.class;
    }
    \end{lstlisting}
