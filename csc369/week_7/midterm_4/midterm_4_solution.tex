...
\begin{enumerate}[1.]
    ...
    \begin{enumerate}[1)]
        ...
        \begin{enumerate}[a)]
            ...
            There is no problem in this case. The symbolic link to file \texttt{mygrades.txt}
            is removed, but the hardlinks to the inode of file \texttt{mygrades.txt} remains.

            \bigskip

            The total number of references to inode of file \texttt{mygrades.txt} is 2.
            ...
            There is no problem at all. The \texttt{mygrade-copy2.txt} is a soft link to file
            \texttt{mygrade-copy.txt} of which points to the inode of file \texttt{mygrade.txt}

            \bigskip

            However, because the file \texttt{mygrade.txt} is removed, the number of references in the inode of \texttt{mygrade.txt}
            would reduce form 2 to 1.
            ...
            So, when removed, \texttt{mygrade-copy2.txt} still points to the inode of \texttt{mygrade.txt}.

            \bigskip

            However, in inode of file \texttt{mygrade.txt}, the total number of reference
            count would reduce from 2 to 1.
        ...
        Check is done by going through each pointers in files and directories to
        make sure that the data blocks match the information in data bitmap.
        ...
        Ommited for now. Topic not in reading list.
    ...
    \begin{enumerate}[a)]
        ...
        The largest file that can be represented by an inode is:

        \bigskip

        $(12 + 1024 + 1024^2) * 4\text{KB} \approx 4.20 \text{GB}$.
        ...
        $12 + (2 \times 1024) + (3 \times 1024^2) \approx 3.15$ million accesses
        are required to read the file of maximum size in sequential order.

        \bigskip

        \item

        The two ways of implementing directories are


        \begin{itemize}
            \item Using lists
            \begin{itemize}
                \item Each directory entry is of format

                \bigskip

                \texttt{("User-readible-file-name", i-number)}
                \item Is easy to implement
                \item Suffers in performance. (we have to check each individual entry to find the correct file)
            \end{itemize}
            \item Using hasmap
            \begin{itemize}
                \item key of hasmap is user-readible file name, and value is i-number of file
                \item Takes space
                \item Excellent in performance.
            \end{itemize}
        \end{itemize}
    \end{enumerate}
    ...
    \begin{enumerate}[a)]
        ...
        The largest possible file is

        \bigskip

        $(12 + 1024 + 1024^2 + 1024^3) * 4\text{KB} \approx 4.30\text{TB}$.

        \item

        A size of block is $4\text{KB}$.

        \bigskip

        So, $(12 + 1024 + 1024^2 + 1024^3) = 1,074,791,436$ blocks are need to
        create this file.
        ...
        \begin{itemize}
            ...
            \begin{itemize}
                \item Can create link across partitions
                \item Can create link to a directory
                \item File system needs to follw the link
                \item Removing the linked file would result in dangling link/reference
            \end{itemize}
            \item \textbf{Hard link}
            \begin{itemize}
                \item Cannot create a link to a directory
                \item Cannot create a link across partitions
                \item No extra work is involved in following the link
                \item Removing hardlink removes file only when its hardlink
                count reaches 0.
            \end{itemize}
