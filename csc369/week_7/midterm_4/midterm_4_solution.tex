    ...
    \begin{enumerate}[a)]
       ...
       \bigskip
        ...
        \begin{itemize}
            ...
            \begin{itemize}
                \item Each write generates five I/Os:

                \begin{itemize}
                    \item Read inode bitmap (to find free inode)
                    \item Write inode bitmap (to mark it allocated)
                    \item Create new inode (to initialize it)
                    \item Create one data block (to link high-level name of file \texttt{bar} to its inode number)
