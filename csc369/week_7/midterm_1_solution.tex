...
\begin{enumerate}[1.]
    ...
    \begin{itemize}
        ...
        \begin{itemize}
            ...
            \begin{enumerate}[1.]
                \item When \texttt{open("/foo/bar", O\_READONLY)} is called, the system first finds the inode
            for the file \texttt{bar} to read basic information about the file

                \item This includes permission, information, file size etc

                \item Begin traversal at the root of the file system, in the \textbf{root directory}

                \item Find \texttt{inode} by

                \begin{itemize}
                    \item Looking for \texttt[i-number]
                \end{itemize}
            \end{enumerate}
        \end{itemize}
    \end{itemize}
