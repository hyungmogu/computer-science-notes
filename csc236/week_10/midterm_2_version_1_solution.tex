...
\begin{itemize}
    ...
    \begin{align}
        H(n): \bigwedge\limits_{i=0}^{n-1} T(i) \leq T(n)
    \end{align}
    ...
    \begin{enumerate}[1.]
        ...
        Since $n \geq 2$, $0 \leq n-1 < n$, so, $H(n-1)$ holds by induction hypothesis.

        \bigskip

        Thus, by transitivity of $\leq$, it is sufficient to only show $T(n-1) \leq T(n)$.

        \bigskip

        Starting with $T(n-1)$, we have
